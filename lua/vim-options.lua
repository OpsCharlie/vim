...
autocmd({ "BufRead", "BufNewFile" }, {
  ...
  --   end, { desc = "Add name prefix", silent = true, buffer = 0 })
  --   vim.keymap.set("n", "<leader>N", ":%s/- name: /- name: %<BS><BS><BS><BS> | /<CR>",
  --     { desc = "Add buffer name prefix", silent = true, buffer = 0 })
  --   vim.keymap.set("n", "<leader>a", "wbiansible.builtin.<ESC>",
  --     { desc = "Add FQCN for builtin module", silent = true, buffer = 0 })
  --   vim.keymap.set('n', '<leader>ca', ':!ansible-lint --fix %<CR>', { desc = "Code Action ansiblelint", buffer = 0 })
  --   -- Set 'K' mapping for every yaml.ansible buffer
